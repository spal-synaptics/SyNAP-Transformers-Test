...
from embeddings.minilm import BaseEmbeddings, embeddings_factory
...
class TextAgent:
    def __init__(
        ...
        qa_file: str, 
        *, 
        embedding_models: list[BaseEmbeddings],
    ...
    def load_embeddings(self, model: BaseEmbeddings, export_embeddings: bool = False) -> np.ndarray:
        if not isinstance(model, BaseEmbeddings):
    ...
    def embed_query(self, query: str, model: BaseEmbeddings) -> tuple[int, np.ndarray]:
...
def compare_text_embeddings(qa_file: str, *, model_configs: list[str | os.PathLike], export_embeddings: bool = False):
    models = [embeddings_factory(config) for config in model_configs]
    ...
    while True:
        ...
        for result in results:
            model, answer, similarity = result["model"], result["answer"], result["similarity"]
            print(YELLOW + f"Answer ({model}): " + answer + RESET)
            print(YELLOW + "Similarity ({model}): " + str(similarity) + RESET)
...
if __name__ == "__main__":
    ...
    model_configs = get_model_configs(args.model_config, ["gguf", "synap"])
